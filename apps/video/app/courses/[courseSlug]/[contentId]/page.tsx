...
// Recursively build sidebar-ready item list from course content
...
export default async function ContentViewerPage({ params }: Props) {
  ...
  // Gate: must have purchased (or course is free with a purchase record)
  ...
  const [progressList, bookmarkList] = await Promise.all([
    ...
    <div className="flex h-[calc(100vh-3.5rem)]">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-4xl px-6 py-8 space-y-6">
          ...
          {/* Notion (placeholder — full renderer in Phase 7) */}
          {content.type === "NOTION" && content.notionMetadata && (
            <div className="rounded-xl border bg-muted/40 p-8 text-center text-muted-foreground">
              Notion content: <code className="font-mono">{content.notionMetadata.notionId}</code>
            </div>
          )}
